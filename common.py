...
def load_process_from_driver(driver, outfile='tmp.py'):
    """
    ...
    process = importlib.import_module(outfile.replace('.py', '')).process
    logger.info('Loaded process %s from %s', process, outfile)
    return process
